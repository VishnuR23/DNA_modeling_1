...
| D77 | Corrected denoiser now returns epsilon directly | Source audit found an objective/sampling mismatch: the old return added noisy coordinates while loss and DDIM interpreted output as epsilon. Fresh regression test and 500-update molecular sanity run confirm consistency; sanity metrics still fail, so this fix is not presented as reproduction. |

## Development update (2026-09-18)

The user authorized revising specifications that block progress. [development_policy.md](development_policy.md) supersedes the Phase 2 implementation prohibition in D69; scientific acceptance thresholds are unchanged. A synthetic variable-size flow prototype is now implemented with independent Gaussian endpoint coupling and Heun integration. This omits TITO optimal-transport alignment and molecular bond features and is explicitly exploratory.

Correction to D77: removing a noisy-input skip is an architectural choice, not by itself a repair of an epsilon-objective mismatch. The full sum can validly predict epsilon while its learned branch predicts a residual. The prior causal explanation was overstated. `model.epsilon_skip` now makes the choice explicit; paired fresh runs compare it. No-skip checkpoint configs lacking this field default to false. Checkpoints created before the historical skip removal still require their original code revision for faithful interpretation.
